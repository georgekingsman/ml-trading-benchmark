...
> 本文档完整记录了「ML Trading Benchmark」项目的全部 **11 项实验**：每项实验 **为什么做（动机）、怎么做（方法）、结果是什么（数据）、如何解读（结论）**。
>
> This document provides exhaustive documentation of all **11 experiments** in the ML Trading Benchmark project: **why** each experiment was conducted, **how** it was implemented, **what** the results are (with exact numbers), and **how** to interpret them.
...
**中文：** 这些实验的目的是为 ESWA 期刊的综述论文「Machine Learning for Quantitative Trading」提供一个**可复现的伴随基准（companion benchmark）**。综述论文在第 10 节讨论了量化交易研究中常见的评估陷阱（只报告 gross 收益、忽略交易成本、不做统计检验、不比较被动基准等），但缺少实际实验来**证明这些陷阱有多严重**。因此我们设计了一套完整的实验管线，从数据下载到模型训练再到回测和统计检验，全程端到端自动化。

**English:** These experiments serve as a **reproducible companion benchmark** for the ESWA survey paper "Machine Learning for Quantitative Trading." The survey's Section 10 discusses common evaluation pitfalls in ML trading research (reporting only gross returns, ignoring transaction costs, lacking statistical tests, omitting passive baselines, etc.) but lacks concrete experiments to **demonstrate how severe these pitfalls are**. We therefore designed a full end-to-end pipeline — from data download through model training, backtesting, and statistical testing — to fill this gap.
...
## 14. 实验 12：Diebold-Mariano 统计检验 / DM Test

### 为什么做 / Why

**中文：** 表 1 显示 LogReg 的 Sharpe 是 0.469 而 RandomForest 是 0.372——看起来 LogReg "更好"。但这个差异在统计上显著吗？还是只是噪音？**Diebold-Mariano (DM) 检验** 是专门用来比较两个时间序列预测模型的统计检验方法。

**English:** Table 1 shows LogReg Sharpe at 0.469 vs RandomForest at 0.372 — LogReg "looks better." But is this difference statistically significant or just noise? The **Diebold-Mariano (DM) test** is specifically designed to compare predictive accuracy of two time series models.
...
**中文：**
1. **66 对中只有 3 对显著（4.5%），接近随机基线的 5%！**
2. 3 对显著对**全部涉及被动基准 SPY**：LSTM 显著差于 SPY（p=0.034），SPY 显著好于 EqualWeight（p=0.002），MeanReversion 显著差于 SPY（p=0.046）。
3. **没有任何 ML-vs-ML 对是显著的。** LogReg vs LightGBM？p=0.537。RandomForest vs MLP？p=0.866。这些"差异"全是噪音。
4. **这是全部实验中最具颠覆性的发现**：大多数 ML 交易论文声称"模型 A 优于模型 B"，但在严格的统计检验下，这些差异根本不存在。
5. 唯一可靠的结论是：**SPY 买入持有在统计上显著优于最差的主动策略（LSTM、均值回归）和等权基准**。

**English:**
1. **Only 3 of 66 pairs significant (4.5%), close to the 5% random baseline!**
2. All 3 significant pairs **involve the passive SPY benchmark**: LSTM significantly worse than SPY (p=0.034), SPY significantly better than EqualWeight (p=0.002), MeanReversion significantly worse than SPY (p=0.046).
...
4. **This is the most disruptive finding of all experiments**: Most ML trading papers claim "Model A outperforms Model B," but under rigorous statistical testing, these differences don't exist.
5. The only reliable conclusion: **SPY buy-and-hold is statistically significantly better than the worst active strategies (LSTM, MeanReversion) and equal-weight**.

---

## 15. 总结 / Summary

### 6 个核心发现 / 6 Core Findings
...
### 对论文的意义 / Implications for the Survey Paper

**中文：** 这些实验完整演示了论文第 10 节讨论的所有评估陷阱。如果一个研究者只看 Table 1 的 gross Sharpe，他会认为 LogisticRegression（0.469）是"不错"的。但加上成本分析（变成 −1.44）、bootstrap CI（包含零）、regime 分解（主要靠 COVID）、DM 检验（和其他模型没有显著差异）之后，结论完全反转：**在 ETF 层面，没有任何 ML 模型在经济和统计意义上都显著优于简单的被动策略。**

**English:** These experiments comprehensively demonstrate every evaluation pitfall discussed in Section 10 of the survey. If a researcher only looks at Table 1's gross Sharpe, they'd think LogisticRegression (0.469) is "decent." But after cost analysis (becomes −1.44), bootstrap CI (includes zero), regime decomposition (mainly driven by COVID), and DM testing (no significant difference from other models), the conclusion completely reverses: **At the ETF level, no ML model is both economically and statistically significantly superior to simple passive strategies.**
...
| `table6_dm_test.csv` | DM 检验：66 对 p-value |
...
> **文档版本 / Document Version**: v1.0, 2026-02-21
> **运行环境 / Runtime**: Python 3.13, macOS ARM (Apple M-series), ~6 min
